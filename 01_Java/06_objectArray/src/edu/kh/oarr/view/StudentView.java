...
public class StudentView {
	...
	public void displayMenu() {
		...
		do {
			...
			System.out.println("5. 학생 점보 수정(인덱스)");
	...
	public void selectScore() {
		...
		System.out.print("조회할 학생의 인덱스를 입력하세요.");
		int index = sc.nextInt();
		
		String s2 = service.selectScore(index);
		
		System.out.println(s2);
		
		/* 학생 정보 수정 (인덱스 번호 입력) + 점수 입력 */
		
		// 1. 인덱스 번호를 입력 받아 service로 전달하여
		//  존재하는 학생이면 학생 객체의 주소, 아니면 null 반환 받기
		
		//2. 존재하지 않는 학생인 경우
		// "해당 인덱스에 학생정도가 존재하지 않습니다." 출력 후 return
		
		// 3. 2번이 실행 되지 않는 경우 ( == 존재 하는 경우)
		//   국어, 영어, 수학 점수를 입력 받아
		...
		public void updateStudent() {
			...
			Student s = service.selectIndex(index);
			//이미 존재하는 service의 selectIndex(index) 메서드 이용
			...
			//  s.setKor(sc.nextInt() ); //입력 받은 점수를 학생 객체에 바로 세팅
	...
	public void selectName() {
		...
		// 이름
		Student[] arr = service.selectName();
